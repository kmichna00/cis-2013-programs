...
if (isNaN(intCount) || intCount < 1 || intCount > 100) //validates the values the user inputs to only allow numbers that we want to work with the program
{
    alert("Your input is not valid. Please pick a number that is between 1-100"); //tells the user to try again if input not valid
    $("total_fib").value = " "; //blanks out where the user put the invalid input
}
    else //runs the loop if user input of number is valid
    {
        while (intCount > 2)
        {
            k = i + j;
  ...
  $("output").value = stringOutput; //posts the result to the DOM  
